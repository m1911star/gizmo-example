...
export class BoundingBoxGizmo extends Gizmo {
     ...
     */
    constructor(color: Color3 = Color3.Red(), gizmoLayer: UtilityLayerRenderer = UtilityLayerRenderer.DefaultKeepDepthUtilityLayer) {
        super(gizmoLayer);
        ...
        // lines.push(LinesBuilder.CreateLines("lines", { points: [new Vector3(this._boundingDimensions.x, this._boundingDimensions.y, this._boundingDimensions.z), new Vector3(0, 0, 0)] }, gizmoLayer.utilityLayerScene));
        const points = [];
        const curve = BABYLON.Curve3.CreateQuadraticBezier(
            ...
            new Vector3(this._boundingDimensions.x,this._boundingDimensions.y,this._boundingDimensions.z*3),
            new Vector3(this._boundingDimensions.x/2,this._boundingDimensions.y/2,this._boundingDimensions.z/2),
            1000
        ).getPoints();
        points.push(...curve);
        const pt = new BABYLON.Path3D(points);
        lines.push(Mesh.CreateLines("lines", pt.getCurve(), gizmoLayer.utilityLayerScene));
        lines.forEach((l) => {
            l.color = color;
